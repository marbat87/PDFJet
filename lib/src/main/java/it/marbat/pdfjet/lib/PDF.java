 ...
 */
@SuppressWarnings({"WeakerAccess", "FieldCanBeLocal", "UnusedReturnValue", "unused"})
public class PDF {
    ...
    private String producer = "PDFjet v5.88 (http://pdfjet.com)";
    ...
    public PDF(OutputStream os, int compliance) throws Exception {
        ...
        Date date = new Date();
        @SuppressLint("SimpleDateFormat") SimpleDateFormat sdf1 = new SimpleDateFormat("yyyyMMddHHmmss");
        @SuppressLint("SimpleDateFormat") SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
    ...
    private int addResourcesObject() throws Exception {
        ...
        if (!fonts.isEmpty() || !importedFonts.isEmpty()) {
            append("/Font\n");
            append("<<\n");
...
*/
    ...
    public Map<Integer, PDFobj> read(InputStream inputStream) throws Exception {
        ...
        Map<Integer, PDFobj> pdfObjects = new TreeMap<>();
    ...
    public List<PDFobj> getPageObjects(
            Map<Integer, PDFobj> objects) throws Exception {
        List<PDFobj> pages = new ArrayList<>();
    ...
    private List<PDFobj> getFontObjects(
            PDFobj resources, Map<Integer, PDFobj> objects) {
        List<PDFobj> fonts = new ArrayList<>();
        List<String> dict = resources.getDict();
        for (int i = 0; i < dict.size(); i++) {
            if (dict.get(i).equals("/Font")) {
                if (!dict.get(i + 2).equals(">>")) {
                    fonts.add(objects.get(Integer.valueOf(dict.get(i + 3))));
                }
            }
        }
    ...
    private List<PDFobj> getDescendantFonts(
        ...
        List<PDFobj> descendantFonts = new ArrayList<>();
    ...
    public void addResourceObjects(Map<Integer, PDFobj> objects) throws Exception {
        Map<Integer, PDFobj> resources = new TreeMap<>();
